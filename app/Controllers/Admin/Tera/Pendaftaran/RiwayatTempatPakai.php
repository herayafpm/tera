...
class RiwayatTempatPakai extends BaseController
{
  ...
  public function index($status)
  {
    ...
    $data['_uri_datatable'] = base_url("admin/tera/pendaftaran/{$this->jenis_tempat_id}/riwayat/{$status}/datatable");
    $data['_batal_tera'] = base_url("admin/tera/pendaftaran/{$this->jenis_tempat_id}/riwayat/{$status}/batal");
    $data['_proses_tera'] = base_url("admin/tera/pendaftaran/{$this->jenis_tempat_id}/riwayat/{$status}/proses");
    $data['_print_tera'] = base_url("admin/tera/pendaftaran/{$this->jenis_tempat_id}/riwayat/{$status}/print_form");
  ...
  public function datatable($status)
  {
    ...
    if ($method == 'post') {
      ...
      $where = ['tera.jenis_tempat_id' => $this->jenis_tempat_id, 'tera.tera_status' => $status];
      $like = null;
      ...
      if (!empty($this->request->getPost('date'))) {
        ...
        $where['tera.tera_date_order >='] = $date[0] . " 00:00:00";
        $where['tera.tera_date_order <='] = $date[1] . " 23:59:59";
      }
  ...
  public function batal($status, $id, $tipe)
  {
    ...
    if ($update) {
      ...
      return redirect()->to(base_url("admin/tera/pendaftaran/{$this->jenis_tempat_id}/riwayat/0"));
    } else {
      $this->session->setFlashdata('error', "Gagal membatalkan {$updateMessage} tera");
      return redirect()->back()->withInput();
    }
  ...
  public function proses($status, $id, $tipe)
  {
    ...
    if ($update) {
      ...
      return redirect()->to(base_url("admin/tera/pendaftaran/{$this->jenis_tempat_id}/riwayat/{$tipe}"));
    } else {
      $this->session->setFlashdata('error', "Gagal {$updateMessage} tera");
      return redirect()->back()->withInput();
    }
  }
  public function print_form($status, $id)
  {
    helper('my');
