...
class HasilPengujian extends BaseController
{
  ...
  public function index($jenis_tempat_id, $status, $tera_id, $tera_uttp_retribusi_id, $tera_uttp_id, $tipe)
  {
    helper('my');
  ...
  protected function pengujian_tipe1($jenis_tempat_id, $status, $tera, $teraUttpRetribusi, $teraUttp)
  {
    ...
    if (!$this->form_validation->run($data)) {
      ...
      $teraUttpModel = new TeraUttpModel();
      if ($teraUttpModel->update($teraUttp['tera_uttp_id'], $data)) {
        $teraUttpDetailModel = new TeraUttpDetailModel();
        $teraUttpDetailModel->where('tera_uttp_id', $teraUttp['tera_uttp_id'])->delete();
        $teraModel = new TeraModel();
  ...
  protected function pengujian_tipe2($jenis_tempat_id, $status, $tera, $teraUttpRetribusi, $teraUttp)
  {
    ...
    if (!$this->form_validation->run($data)) {
      ...
      if ($teraUttpModel->update($teraUttp['tera_uttp_id'], $data)) {
        $teraUttpDetailModel = new TeraUttpDetailModel();
        $teraUttpDetail = $teraUttpDetailModel->where(['tera_uttp_id' => $teraUttp['tera_uttp_id']])->first();
        if ($teraUttpDetail) {
          $teraUttpDetailModel->update($teraUttpDetail['tera_uttp_detail_id'], $data);
        } else {
          $data['tera_uttp_id'] = $teraUttp['tera_uttp_id'];
  ...
  public function print_hasil($jenis_tempat_id, $status, $tera_id, $tera_uttp_retribusi_id, $tera_uttp_id, $tipe)
  {
    helper('my');
    $data['_view'] = "admin/tera/pengujian/hasil_pengujian/{$tipe}/print";
    $teraModel = new TeraModel();
    ...
    $teraUttpRetribusiModel = new TeraUttpRetribusiModel();
    $teraUttpRetribusi = $teraUttpRetribusiModel->getTeraUttp($tera_uttp_retribusi_id);
    $data['_title'] = "Print Surat Keterangan Hasil Pengujian";
