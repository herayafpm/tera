    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="form-group row">
          <div class="col-md-4"><input class="form-control form-control-sm" type="text" id="jabatan_nama" placeholder="Jabatan (min. 3 karakter)" /></div>
        </div>
...
<script>
  ...
  function ubah(id) {
    window.location.href = "<?= base_url('admin/master/jabatan/ubah') ?>/" + id;
  }

  function hapus(id) {
    hapusData({
      fun: function() {
        window.location.href = "<?= base_url('admin/master/jabatan/hapus') ?>/" + id
      }
    })
  }
  $(function() {
    tabel = $("#datatable").DataTable({
      ...
      "buttons": [{
        ...
        action: function(e, dt, node, config) {
          window.location.href = "<?= base_url('admin/master/jabatan/tambah') ?>"
        }
      }, {
      ...
      "columns": [{
        ...
        },
        {
          "render": function(data, type, row, meta) { // Tampilkan kolom aksi
            var html = '<button type="button" class="btn btn-link text-info" onClick="ubah(' + row.jabatan_id + ')"><i class="fa fa-fw fa-edit" aria-hidden="true" title="Edit ' + row.jabatan_nama + '"></i></button>'
            html += '<form method="POST" class="d-inline deleteData"><button type="button" class="btn btn-link text-danger" onClick="hapus(' + row.jabatan_id + ')"><i class="fa fa-fw fa-trash" aria-hidden="true" title="Hapus ' +
              row.jabatan_nama + '"></i></button></form>'
            return html
          }
        },
      ],
    ...
    $('#jabatan_nama').on('keyup change clear', function() {
      var value = $(this).val();
      var min_karakter = 3;
      if (value.length >= min_karakter) {
        data.jabatan_nama = value;
      }
