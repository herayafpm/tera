...
<script>
  ...
  function ubah(id) {
    window.location.href = "<?= base_url('admin/master/jenis_uttp_retribusi/ubah') ?>/" + id;
  }
  $(function() {
      ...
      "columns": [{
        ...
        },
        {
          "render": function(dt, type, row, meta) { // Tampilkan kolom aksi
            var html = "<ul>";
            if (row.jenis_uttp != null) {
              row.jenis_uttp.forEach((item, index) => {
                html += "<li>" + item.jenis_uttp_nama + "</li>"
              })
            }
            html += "</ul>"
            return html
        ...
        {
          "render": function(dt, type, row, meta) { // Tampilkan kolom aksi
            var html = '<button type="button" class="btn btn-link text-info" onClick="ubah(' + row.jenis_retribusi_tipe_id + ')"><i class="fa fa-fw fa-edit" aria-hidden="true" title="Edit Jenis UTTP Retribusi' + row.jenis_retribusi_tipe_nama + '"></i></button>'
            return html
          }
        },
      ],
