    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="form-group row">
          <div class="col-md-4"><input class="form-control form-control-sm" type="text" id="jenis_uttp_tipe_nama" placeholder="Nama Tipe (min. 3 karakter)" /></div>
        </div>
...
<script>
  ...
  function ubah(id) {
    window.location.href = "<?= base_url('admin/master/jenis_uttp_tipe/ubah') ?>/" + id;
  }

  function hapus(id) {
    hapusData({
      fun: function() {
        window.location.href = "<?= base_url('admin/master/jenis_uttp_tipe/hapus') ?>/" + id
      }
    })
  }
  $(function() {
    tabel = $("#datatable").DataTable({
      ...
      "buttons": [{
        ...
        action: function(e, dt, node, config) {
          window.location.href = "<?= base_url('admin/master/jenis_uttp_tipe/tambah') ?>"
        }
      }, {
      ...
      "columns": [{
          "data": "jenis_uttp_tipe_id",
        }, {
          "data": "jenis_uttp_tipe_nama",
        },
        {
          "render": function(data, type, row, meta) { // Tampilkan kolom aksi
            var html = '<button type="button" class="btn btn-link text-info" onClick="ubah(' + row.jenis_uttp_tipe_id + ')"><i class="fa fa-fw fa-edit" aria-hidden="true" title="Edit ' + row.jenis_uttp_tipe_nama + '"></i></button>'
            html += '<form method="POST" class="d-inline deleteData"><button type="button" class="btn btn-link text-danger" onClick="hapus(' + row.jenis_uttp_tipe_id + ')"><i class="fa fa-fw fa-trash" aria-hidden="true" title="Hapus ' +
              row.jenis_uttp_tipe_nama + '"></i></button></form>'
            return html
          }
        },
      ],
    ...
    $('#jenis_uttp_tipe_nama').on('keyup change clear', function() {
      var value = $(this).val();
      var min_karakter = 3;
      if (value.length >= min_karakter) {
        data.jenis_uttp_tipe_nama = value;
      }
