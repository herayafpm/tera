    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="form-group row">
          <div class="col-md-4">
            <select class="form-control form-control-sm" id="tera_keringanan_status">
...
<div class="modal fade" id="detailTeraKeringananModal" tabindex="-1" role="dialog" aria-labelledby="detailTeraKeringananModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="detailTeraKeringananModalLabel">Detail Keringanan</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <div class="row">
          ...
          <div class="col">
            : <span class="tera_keringanan_status"></span>
          </div>
        </div>
        <div class="row tera_keringanan_status_verif_row">
          <div class="col-4">
            Diverifikasi
          </div>
          <div class="col">
            : <span class="tera_keringanan_status_verif"></span>
          </div>
        </div>
        <div class="row tera_keringanan_status_tolak_row">
          <div class="col-4">
            Ditolak
          </div>
          <div class="col">
            : <span class="tera_keringanan_status_tolak"></span>
          </div>
        </div>
        <div class="row">
          <div class="col-4">
          ...
          <div class="col">
            : <span class="tera_keringanan_keterangan"></span>
          </div>
        </div>
        <div class="row">
          <div class="col-4">
          ...
          <div class="col">
            : <span class="tera_keringanan_date"></span>
          </div>
        </div>
      </div>
      <div class="modal-footer">
...
<script>
  ...
  function verif(id) {
    Swal.fire({
      title: "Konfirmasi Verifikasi Pengajuan Keringanan Tera",
      text: "Yakin ingin memverifikasi Pengajuan Keringanan ini?",
      icon: 'warning',
  ...
  function batal(id, tipe) {
    Swal.fire({
      title: "Konfirmasi Membatalkan Status Pengajuan Keringanan Tera",
      text: "Yakin ingin Membatalkan Status Pengajuan Keringanan ini?",
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
  ...
  function tolak(id) {
    Swal.fire({
      title: "Konfirmasi Menolak Pengajuan Keringanan Tera",
      text: "Yakin ingin Menolak Pengajuan Keringanan ini?",
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
  ...
  function detailTeraKeringanan(index) {
    var keringanan = datas[index];
    $('.tera_keringanan_status_verif_row').addClass('d-none');
    $('.tera_keringanan_status_tolak_row').addClass('d-none');
    var status = "Proses";
    if (keringanan.tera_keringanan_status == 1) {
      if (keringanan.tera_keringanan_status_by != null) {
        $('.tera_keringanan_status_verif_row').removeClass('d-none');
        $('.tera_keringanan_status_verif').html(keringanan.admin_nama + " Pada " + toLocaleDate(keringanan.tera_keringanan_status_at, 'LLL'))
      }
      status = "Verifikasi"
    } else if (keringanan.tera_keringanan_status == 2) {
      if (keringanan.tera_keringanan_status_by != null) {
        $('.tera_keringanan_status_tolak_row').removeClass('d-none');
        $('.tera_keringanan_status_tolak').html(keringanan.admin_nama + " Pada " + toLocaleDate(keringanan.tera_keringanan_status_at, 'LLL'))
      }
      status = "Ditolak"
    }
    $('.tera_keringanan_status').html(status);
    $('.tera_keringanan_keterangan').html(keringanan.tera_keringanan_keterangan);
    $('.tera_keringanan_date').html(toLocaleDate(keringanan.tera_keringanan_date, 'LLL'));
    $('#detailTeraKeringananModal').modal()
  }
  ...
  $(function() {
    ...
    tabel = $("#datatable").DataTable({
      ...
      "columns": [{
          "data": "tera_keringanan_id",
        }, {
          "data": "tera_keringanan_status",
          "render": function(data, type, row, meta) { // Tampilkan kolom aksi
            var status = "Proses"
            if (row.tera_keringanan_status == 1) {
              status = "Diverif " + row.admin_nama
            } else if (row.tera_keringanan_status == 2) {
              status = "Ditolak " + row.admin_nama
            }
            return status;
          }
        }, {
          "data": "tera_keringanan_date",
          "render": function(data, type, row, meta) { // Tampilkan kolom aksi
            return toLocaleDate(row.tera_keringanan_date)
          }
        },
        {
          "render": function(data, type, row, meta) { // Tampilkan kolom aksi
            var html = '<button type="button" class="btn btn-link text-primary" onClick="detailTeraKeringanan(' + meta.row + ')"><i class="fa fa-fw fa-eye" aria-hidden="true" title="Detail Keringanan Tera"></i></button>'
            if (row.tera_keringanan_status == 0) {
              html += '<button type="button" class="btn btn-link text-success" onClick="verif(' + row.tera_keringanan_id + ')"><i class="fa fa-fw fa-check" aria-hidden="true" title="Verif Keringanan Tera"></i></button>'
              html += '<button type="button" class="btn btn-link text-danger" onClick="tolak(' + row.tera_keringanan_id + ')"><i class="fa fa-fw fa-times" aria-hidden="true" title="Tolak Keringanan Tera"></i></button>'
            } else {
              var text = "Tolak"
              var tipe = 2;
              if (row.tera_keringanan_status == 1) {
              ...
              html += '<button type="button" class="btn btn-link text-danger" onClick="batal(' + row.tera_keringanan_id + ',' + tipe + ')"><i class="fa fa-fw fa-times" aria-hidden="true" title="Batal ' + text + ' Keringanan Tera"></i></button>'
            }
            return html
          }
        },
    ...
    $('#tera_keringanan_status').on('change clear', function() {
      var value = $(this).val();
      data.tera_keringanan_status = value;
