...
<script>
  ...
  function skrd(id) {
    window.location.href = "<?= $_skrd ?>/" + id
  }

  function skrdkb(id) {
    window.location.href = "<?= $_skrdkb ?>/" + id
  }

  function skrdlb(id) {
    window.location.href = "<?= $_skrdlb ?>/" + id
  }

  function ssrd(id) {
    window.location.href = "<?= $_ssrd ?>/" + id
  }

  function keringanan(id) {
    window.location.href = "<?= $_keringanan ?>/" + id
  }
      ...
      "columns": [{
        ...
        },
        {
          "render": function(data, type, row, meta) { // Tampilkan kolom aksi
            ...
            html += '<button type="button" class="btn btn-link text-info" onClick="skrd(' + row.tera_id + ')">SKRD</button>'
            html += '<button type="button" class="btn btn-link text-info" onClick="keringanan(' + row.tera_id + ')">Keringanan</button>'
            html += '<button type="button" class="btn btn-link text-info" onClick="skrdkb(' + row.tera_id + ')">SKRDKB</button>'
            html += '<button type="button" class="btn btn-link text-info" onClick="skrdlb(' + row.tera_id + ')">SKRDLB</button>'
            html += '<button type="button" class="btn btn-link text-info" onClick="ssrd(' + row.tera_id + ')">SSRD</button>'
            return html
          }
        },
      ],
