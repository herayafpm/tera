  ...
  <div class="col-12">
    <div class="card">
      ...
      <div class="card-body">
        ...
        <?php if ($_tera_skrdkb_at != null) : ?>
          <a href="<?= $_url_print ?>" target="_blank" class="btn btn-success mb-1" role="button"><i class="fa fa-print"></i> Cetak SKRDKB</a>
        <?php endif ?>
        ...
        <div class="form-group row mt-2">
          <label for="tera_skrdkb_terbilang" class="col-md-2 col-form-label">SKRDKB Terbilang</label>
          <div class="col-md-8">
            <textarea class="form-control form-control-sm <?= ($_validation->hasError('tera_skrdkb_terbilang') ? "is-invalid" : "") ?>" id="tera_skrdkb_terbilang" rows="3" name="tera_skrdkb_terbilang" placeholder="Masukkan Terbilang SKRDKB"><?= old('tera_skrdkb_terbilang', $_tera_skrdkb_terbilang) ?></textarea>
            <div class="invalid-feedback">
              <?= $_validation->getError('tera_skrdkb_terbilang') ?>
            </div>
          </div>
        </div>
        ...
        <div class="form-group row">
          <div class="col-md-4">
            <select class="form-control form-control-sm" id="tera_skrdkb_status">
...
<div class="modal fade" id="detailSKRDKBModal" tabindex="-1" role="dialog" aria-labelledby="detailSKRDKBModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="detailSKRDKBModalLabel">Detail SKRDKB</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <div class="row">
          ...
          <div class="col">
            : <span class="tera_skrdkb_status"></span>
          </div>
        </div>
        <div class="row tera_skrdkb_status_verif_row">
          <div class="col-4">
            Diverifikasi
          </div>
          <div class="col">
            : <span class="tera_skrdkb_status_verif"></span>
          </div>
        </div>
        <div class="row tera_skrdkb_status_tolak_row">
          <div class="col-4">
            Ditolak
          </div>
          <div class="col">
            : <span class="tera_skrdkb_status_tolak"></span>
          </div>
        </div>
        <div class="row">
          <div class="col-4">
          ...
          <div class="col">
            : <span class="tera_skrdkb_keterangan"></span>
          </div>
        </div>
        <div class="row">
          <div class="col-4">
          ...
          <div class="col">
            : <span class="tera_skrdkb_date"></span>
          </div>
        </div>
      </div>
      <div class="modal-footer">
...
<script>
  ...
  function detailSKRDKB(index) {
    ...
    $('.tera_skrdkb_status_verif_row').addClass('d-none');
    $('.tera_skrdkb_status_tolak_row').addClass('d-none');
    var status = "Proses";
    if (skrdkb.tera_skrdkb_status == 1) {
      if (skrdkb.tera_skrdkb_status_by != null) {
        $('.tera_skrdkb_status_verif_row').removeClass('d-none');
        $('.tera_skrdkb_status_verif').html(skrdkb.admin_nama + " Pada " + toLocaleDate(skrdkb.tera_skrdkb_status_at, 'LLL'))
      }
      status = "Verifikasi"
    } else if (skrdkb.tera_skrdkb_status == 2) {
      if (skrdkb.tera_skrdkb_status_by != null) {
        $('.tera_skrdkb_status_tolak_row').removeClass('d-none');
        $('.tera_skrdkb_status_tolak').html(skrdkb.admin_nama + " Pada " + toLocaleDate(skrdkb.tera_skrdkb_status_at, 'LLL'))
      }
      status = "Ditolak"
    }
    $('.tera_skrdkb_status').html(status);
    $('.tera_skrdkb_keterangan').html(skrdkb.tera_skrdkb_keterangan);
    $('.tera_skrdkb_date').html(toLocaleDate(skrdkb.tera_skrdkb_date, 'LLL'));
    $('#detailSKRDKBModal').modal()
  }
  ...
  $(function() {
    ...
    tabel = $("#datatable").DataTable({
      ...
      "columns": [{
          ...
          "data": "tera_skrdkb_status",
          "render": function(data, type, row, meta) { // Tampilkan kolom aksi
            var status = "Proses"
            if (row.tera_skrdkb_status == 1) {
              status = "Diverif " + row.admin_nama
            } else if (row.tera_skrdkb_status == 2) {
              status = "Ditolak " + row.admin_nama
            }
            return status;
          }
        }, {
          "data": "tera_skrdkb_date",
          "render": function(data, type, row, meta) { // Tampilkan kolom aksi
            return toLocaleDate(row.tera_skrdkb_date)
          }
        },
        {
          "render": function(data, type, row, meta) { // Tampilkan kolom aksi
            ...
            if (row.tera_skrdkb_status == 0) {
              html += '<button type="button" class="btn btn-link text-success" onClick="verif(' + row.tera_skrdkb_id + ')"><i class="fa fa-fw fa-check" aria-hidden="true" title="Verif SKRDKB"></i></button>'
              html += '<button type="button" class="btn btn-link text-danger" onClick="tolak(' + row.tera_skrdkb_id + ')"><i class="fa fa-fw fa-times" aria-hidden="true" title="Tolak SKRDKB"></i></button>'
            } else {
              var text = "Tolak"
              var tipe = 2;
              if (row.tera_skrdkb_status == 1) {
              ...
              html += '<button type="button" class="btn btn-link text-danger" onClick="batal(' + row.tera_skrdkb_id + ',' + tipe + ')"><i class="fa fa-fw fa-times" aria-hidden="true" title="Batal ' + text + ' SKRDKB"></i></button>'
            }
            return html
          }
        },
    ...
    $('#tera_skrdkb_status').on('change clear', function() {
      var value = $(this).val();
      data.tera_skrdkb_status = value;
