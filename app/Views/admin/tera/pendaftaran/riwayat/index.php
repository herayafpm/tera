...
<script>
  ...
  function batalVerif(id) {
    Swal.fire({
      ...
      text: "Yakin ingin membatalkan verifikasi tera ini?",
      icon: 'warning',
  ...
  function batalTolak(id) {
    Swal.fire({
      title: "Konfirmasi Batalkan Penolakan Tera",
      text: "Yakin ingin membatalkan penolakan tera ini?",
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
  ...
  async function verif(id) {
    const {
      ...
      inputLabel: 'No Order',
      inputPlaceholder: 'Masukkan No Order'
    })

    if (no_order) {
      Swal.fire({
        title: "Konfirmasi Verifikasi",
        text: "Yakin ingin verifikasi tera ini?",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        ...
        if (result.isConfirmed) {
          window.location.href = "<?= $_proses_tera ?>/" + id + "/1?tera_no_order=" + no_order;
        }
      })
    }
  ...
  function tolak(id) {
    Swal.fire({
      title: "Konfirmasi Penolakan Tera",
      text: "Yakin ingin menolak tera ini?",
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
  ...
  function printForm(id) {
    window.open("<?= $_print_tera ?>/" + id, '_blank');
  }
      ...
      "columns": [{
        ...
        },
        {
          "render": function(data, type, row, meta) { // Tampilkan kolom aksi
            ...
            if (row.tera_status == 0 && row.jenis_tempat_id == 1) {
              html += '<button type="button" class="btn btn-link text-success" onClick="verif(' + row.tera_id + ')"><i class="fa fa-fw fa-check" aria-hidden="true" title="Verif"></i></button>'
              html += '<button type="button" class="btn btn-link text-danger" onClick="tolak(' + row.tera_id + ')"><i class="fa fa-fw fa-times" aria-hidden="true" title="Tolak"></i></button>'
            }
            if (row.tera_status_bayar == 0 && row.jenis_tempat_id == 1) {
              if (row.tera_status_by == "<?= $_admin->admin_id ?>") {
                if (row.tera_status == 1) {
                  html += '<button type="button" class="btn btn-link text-success" onClick="printForm(' + row.tera_id + ')"><i class="fa fa-fw fa-print" aria-hidden="true" title="Print Formulir"></i></button>'
                  html += '<button type="button" class="btn btn-link text-danger" onClick="batalVerif(' + row.tera_id + ')"><i class="fa fa-fw fa-times" aria-hidden="true" title="Batal Verif"></i></button>'
                } else if (row.tera_status == 2) {
                  html += '<button type="button" class="btn btn-link text-danger" onClick="batalTolak(' + row.tera_id + ')"><i class="fa fa-fw fa-times" aria-hidden="true" title="Batal Tolak"></i></button>'
                }
              }
            }
            return html
